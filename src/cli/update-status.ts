...
export const updateStatusCommand = new Command("update-status")
  ...
  .requiredOption("-s, --status <status>", "新状态: pending|ready|running|passed|failed|blocked|cancelled")
  .action((options) => {
    const rootDir = process.cwd();
    const status = options.status as NodeStatus;
    if (!Object.values(NodeStatus).includes(status)) {
      console.error(`❌ 非法状态: ${options.status}。允许的值: ${Object.values(NodeStatus).join(", ")}`);
      process.exit(1);
    }
    try {
