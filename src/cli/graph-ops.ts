...
// arch-c1（C1）全迁：工作区级命令（workspace 模式跳过图目录解析），参数校验改抛
// CliUsageError（消息与拆钩前逐字一致），错误渲染/退出码归 runner。
...
/** 目标图不存在（switch 通道）：含可用列表 + did-you-mean 提示，消息与拆钩前一致 */
function throwGraphMissingForSwitch(name: string, names: string[]): never {
...
export const switchCommand = defineCommand("switch", { workspace: true }).alias("sw")
  ...
  .action((name: string | undefined) => {
    ...
    if (!names.includes(name)) {
      throwGraphMissingForSwitch(name, names);
    }
...
export const listGraphsCommand = defineCommand("list", { workspace: true }).alias("ls")
  .description("列举工作区全部图（或查指定图详情）")
  .argument("[name]", "图名（缺省列全部）")
  .option("--json", "输出稳定 JSON")
  .action((name: string | undefined, options: { json?: boolean }, _cmd, ctx) => {
    const wsRoot = process.cwd();
    const names = listGraphNames(wsRoot);
    if (names.length === 0) {
      throw new CliUsageError(`${wsRoot}/.graph/ 下没有任何图，先 graph init <内容名> -l "<显示名>"`);
    }
    const cur = cliGraphCtx(wsRoot);
    if (name !== undefined) {
      if (!names.includes(name)) {
        throw new CliUsageError(`图 "${name}" 不存在。可用: ${names.join(", ")}`);
      }
      const s = summarize(wsRoot, name);
      ctx.emit(
        () => ({ ...s, isCurrent: name === cur.name, dir: graphDirOf(wsRoot, name) }),
        () => {
          ctx.out(`图: ${s.name}${name === cur.name ? "（当前）" : ""}`);
          ctx.out(`  显示名: ${s.label}`);
          ctx.out(`  节点: ${s.nodeCount}（running ${s.running} / passed ${s.passed}）｜边: ${s.edgeCount}`);
          ctx.out(`  最近活动: ${s.lastActivity ?? "无"}`);
          ctx.out(`  目录: ${graphDirOf(wsRoot, name)}`);
        },
      );
      return;
    }
    ...
    ctx.emit(
      () => ({ current: cur.name, graphs: rows }),
      () => {
        ctx.out(`图: ${cur.name}（当前）｜共 ${rows.length} 张`);
        ctx.out("名                当前  节点  running/passed  最近活动");
        ctx.out("────────────────  ────  ────  ──────────────  ─────────────────");
        for (const r of rows) {
          ctx.out(
...
export const renameGraphCommand = defineCommand("rename-graph", { workspace: true }).alias("rg")
  .description("重命名图（rename 目录 + 修正 active + 记审计）；仅 CLI 人类通道")
  .requiredOption("-o, --old <name>", "旧图名")
  .requiredOption("-n, --new <name>", "新图名（内容命名）")
  .action((options: { old: string; new: string }) => {
    const wsRoot = process.cwd();
    ...
    if (names.includes(options.new)) {
      throw new CliUsageError(`新图名 "${options.new}" 已存在`);
    }
...
export const deleteGraphCommand = defineCommand("delete-graph", { workspace: true }).alias("dg")
  .description("删除图（软删除至 .graph/.trash/<名>-<时间戳>/，可手工救回）；仅 CLI 人类通道")
  .requiredOption("-i, --id <name>", "图名")
  .option("--confirm", "确认删除（缺省拒绝并列出将删除的内容）")
  .action((options: { id: string; confirm?: boolean }) => {
    const wsRoot = process.cwd();
    const names = listGraphNames(wsRoot);
    ...
    if (names.length === 1) {
      throw new CliUsageError(`拒绝删除最后一张图（"${options.id}" 是工作区仅剩的图）`);
    }
    ...
    if (!options.confirm) {
      // 交互确认门：列出将删内容后以退出码 1 终止（stdout 通道与拆钩前一致）
