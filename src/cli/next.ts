...
// arch-c1（C1）：纯格式化函数（无 I/O、无 exit），action 只做装配。
  ...
  .action((options: { staleMs?: string }, _cmd, ctx: RunContext) => {
    ...
    ctx.emit(
      ...
      () => {
        ...
        ctx.out(`\n✅ 可认领 (ready):`);
        if (result.ready.length === 0) ctx.out(`   （无）`);
        for (const n of result.ready) {
          ctx.out(`   ${n.id}: ${n.label}${n.waiting_human ? " 🧑 等真人" : ""}`);
        }
        ctx.out(`\n🔓 可转 ready (门禁已满足):`);
        if (result.ready_eligible.length === 0) ctx.out(`   （无）`);
        ...
        ctx.out(`\n⏳ 等依赖 (blocked 候选):`);
        if (result.blocked.length === 0) ctx.out(`   （无）`);
        ...
        ctx.out(`\n🏃 执行中 (running):`);
        if (result.running.length === 0) ctx.out(`   （无）`);
        ...
        if (result.stale_running.length > 0) {
          ...
          for (const n of result.stale_running) {
            ctx.out(`   ${n.id}: ${n.label} (${Math.round(n.elapsed_ms / 60000)} 分钟)`);
          }
        }
