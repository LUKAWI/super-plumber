...
## [1.0.0] — 2026-09-07（stable：协议冻结与全量验证完成）

> 稳定版：统一 `/plumber` 路由、多图读面、模板与纪律技能、MCP/CLI v1 协议，以及 debug/性能改进均已完成验证；Q1-Q9 九项指标全部通过。

- **协议冻结**：MCP `graph_*` 与 `graph` CLI 的 v1 工具/命令、参数、返回字段、状态机、门禁和错误语义进入稳定承诺；破坏性变化改走下一 major。
- **交付闭环**：统一入口、多图巡检、模板初始化、雾区与档位可观测性、CLI/MCP 双通道和 canonical/generated 同步门禁全部收口。
- **性能与质量**：Q1 next/validate 基准、Q2 WebUI 1k 图交互、Q4 MCP 描述预算、全量后端/WebUI 测试与 Windows/Linux CI 门禁均通过。
- **发布定位**：npm `latest` 指向 `1.0.0`；`1.0.0-beta` 保留为历史预发布版本记录。

## [1.0.0-beta] — 2026-09-06（预发布：1.0.0 协议与统一入口候选）

> 预发布版：集中验证统一 `/plumber` 路由、多图巡检、模板初始化、纪律技能和 MCP/CLI v1 协议承诺；Q1-Q9 仍保留未通过项，本版不代表 1.0.0 stable。

- **统一入口与多图读面**：增加 `/plumber` 路由提示、`survey` 巡检、跨图 `next --all` 和模板初始化能力。
- **协议与集成收口**：补齐纪律族、MCP/CLI 对账、生成产物同步和 1.0.0 deprecation 约定。
- **发布定位**：npm 使用 `beta` dist-tag，供当前候选能力与 Q1-Q9 标准重新评估。
