...
{{#pi}}<!-- @lukawi/super-plumber · gen 产物（pi 渠道视图，0.9.4 S01 单真相源重组起由 integrations/src/ 生成，手改会被 gen --check 拦截）。语法一律查 integrations/shared/manual.md -->{{/pi}}{{#plugin}}<!-- @lukawi/super-plumber · gen 产物（插件包渠道视图，claude code 与 zcode 通用，0.9.4 S01 单真相源重组起由 integrations/src/ 生成，手改会被 gen --check 拦截）。语法一律查 ./manual.md -->{{/plugin}}
...
接单第一步：Read `{{manual}}`（相对{{base}}）§2 与 §6——本定义不含调用语法与操作细节，一切以手册为准。
