...
1. **工作类定档**（这个活是什么量级）→ 回 plumber-design **Phase 0 路由表**顺序定档：关键未知阻止形成可信交付计划则 program；否则按一个会话能否完成并验收选 quick / standard，跨会话或跨图不单独触发 program（DEC-2）：{{#pi}}`.pi/skills/plumber-design/SKILL.md`。{{/pi}}{{#plugin}}本插件包 `./skills/plumber-design/SKILL.md`（pi 渠道为 `.pi/skills/plumber-design/SKILL.md`）。{{/plugin}}
