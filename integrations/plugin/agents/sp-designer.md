...
**输出自检（交付前最后一道）**：上面报告的「领域」段**不可省略**——context 0 个 / ADR 0 篇时也必须显式写 0 并附一句跳过理由（单关注点小任务合法跳过建模，但不合法跳过汇报）；**报告缺领域段 = 未完成**，不算交付，补齐领域段后重新输出才算完成（C1 防线，对应 issue log v0.8.0 C1）。
...
### 附录 · 边类型选型判据（IL-011 收敛：默认 depends_on，不逐边判断）

| 类型 | 用在哪 | 参与排序 | 设计期判据 |
|------|--------|:---:|------|
| `depends_on` | **默认与日常唯一选择**：B 必须等 A 完成 | ✅ | 并行 = 同一 source 多条出边；汇合 = 同一 target 多条入边——ready 门禁天然要求全部前驱 passed，无需专用边型 |
| `decides` | 决策管辖：ADR → 它决定的节点/context | ❌ | **仅 ADR 建模阶段出现**；superseded 时沿此传播 adr_flags；source 必须是 adr 顶点 |
| `relates` | 领域关系：context ↔ context | ❌ | **仅领域建模阶段出现**；rel_kind 自由标注，仅限两端都是 context 顶点 |
| `shares_context` | 共享上下文：A、B 共享一份材料 | ❌ | 非门禁标注（不参与排序与门禁）——语义真有时才显式写 |
| `validates` | 验证关系：B 验证 A 的产物 | ✅ | **向后兼容存量、新设计不再使用**——机器行为与 depends_on 同构 |
| `fan_out` / `fan_in` | 并行分发 / 汇聚合并 | ✅ | **向后兼容存量、新设计不再使用**——门禁语义与多条 depends_on 出/入边等价 |
| `fallback` / `iterates` | 失败兜底 / 迭代优化 | ❌ | **保留字禁用**——运行时未实现其语义（文档性标注，validate 逐条警告） |

**方向约定（写反 = 门禁倒挂）**：箭头从前置指向依赖者——source 是被依赖的前置，target 是等待方；ready 门禁校验的是 target 对 source 的等待。（实证：2026-08-30 主控绑定 IL 时写反三条边，若非复查将静默卡死下游）

**选型自问只剩三条**：① 工作流节点间有依赖？→ `depends_on`（type 可省略，MCP/CLI 缺省即它）。② 这条 ADR 管辖谁？→ `decides`；两个 context 什么关系？→ `relates`（+ rel_kind）。③ 真的共享一份材料且不该阻塞？→ `shares_context`。

> 反模式：给"并行/汇聚"用 fan 边、给"验证"用 validates（新设计一律 depends_on 多边表达）；给知识语义发明新边类型（一种类型+自由标注够了）。语法与全类型约束见手册 §3——本表只管"什么时候用哪种"，不重复语法。
