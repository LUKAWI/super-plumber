...
- 统一入口边界：用户主动要求拷问/意图对齐时路由到 SP 自带 `sp-grilling`；`diagnosing` / `prototype` 保留为 1.0.x 后续，本期不新增、不发命令。
...
三层验收逐层手法与命令语法 → `Read ./manual.md §8、§6、§9`（claude/zcode 插件包环境按手册 §11 寻址约定改为包根相对路径）。
