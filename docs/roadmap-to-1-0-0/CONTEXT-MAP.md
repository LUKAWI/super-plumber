...
| ctx-phase-100（开发顺序④：1.0.0 收口期） | 索引型上下文（分期域，不承载节点归属）：盘点 1.0.0 收口期全部工作——统一入口 /plumber router + 纪律族补全（grilling-lite | 1 | contexts/ctx-phase-100.md |
...
| ctx-webui（呈现层（web-ui 星空）） | 负责 web-ui/（只读星空前端）的呈现层：星图与图库、决策文档入口、前沿视图、雾区云团、术语 Avoid 呈现。不负责机器面与读接口实现（归 ctx-too | 3 | contexts/ctx-webui.md |
